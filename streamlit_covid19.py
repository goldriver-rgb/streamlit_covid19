...
# 페이지 설정
st.set_page_config(page_title='코로나19 한구 대시보드', layout='wide')
...
if uploaded_confirmed and uploaded_deaths and uploaded_recovered:
    ...
    def get_korea_data(df, value_name): # 매개변수 df -> 데이터 프레임, value_name -> 확진자, 사망자, 회복자
    ...
    # .diff() --> 차이(오늘 - 어제)
    # .fillna(0) --> 첫번째날ㅇ느 배교대상이 없으니 NaN -> 0으로 채운다
    ...
    with tab1:
        ...
        if selected: # 선택된 값인 있다면 그래프를 그린다
        ...
        selected_new = st.multiselect('표시할 항목 (신규)을 선택하세요', ['신규 확진자', '신규 사망자', '신규 회복자'], default = ['신규 확진자'])
        if selected_new:
    ...
    with tab3:
        ...
        # 확진자 -> confirmed, 사망자 -> deaths, 회복자 -> recovered
        confirmed = latest['확진자']
        deaths = latest['사망자']
        recovered = latest['회복자']
        
        # 조건 연산자 -> 결과를 담는 변수 = 참일 경우 값 if 조건식 esle 거짓일 경우 값
        # 회복률 = (회복자 / 확진자) * 100
